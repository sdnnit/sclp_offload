# Makefile for sclp_offload driver
# 
...
TARGET:= sclp-offload.ko

CFILES = sclp_offload.c

sclp-offload-objs:= $(CFILES:.c=.o)

all: ${TARGET}

sclp-offload.ko: $(CFILES)
	make -C /lib/modules/`uname -r`/build M=`pwd` V=1 modules

clean:
	make -C /lib/modules/`uname -r`/build M=`pwd` V=1 clean

obj-m:= sclp_offload.o

sclp_offload.c: sclp.h
